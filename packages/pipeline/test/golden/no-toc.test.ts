...
// 3-page PDF with no TOC — detectToc returns 'no' for all pages
...
function buildStubs(): Map<string, { text: string }> {
  ...
  // Step 02: detectToc — all return 'no'
  ...
  // Fallback: processNoToc → noTocHeadings (all pages in one chunk, HIERARCHICAL_PROCESSING=false)
...
beforeEach(async () => {
  dataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'golden-notoc-'));
...
describe('golden: no-TOC fallback', () => {
  it('produces a valid DocOutput tree via no-toc path', async () => {
